      ...
      context("commands", function() {
        ...
        describe("generate-runners", function() {
          ...
          it("can set the Github Actions output to `runner-variables`", async function() {
            ...
            const actual =
              output.includes(`::set-output name=runner-variables::["1/4","2/4","3/4","4/4"]`) ||
              output.includes(`Set output runner-variables = ["1/4","2/4","3/4","4/4"]`) ||
              stderrOutput.includes(`::set-output name=runner-variables::["1/4","2/4","3/4","4/4"]`) ||
              stderrOutput.includes(`Set output runner-variables = ["1/4","2/4","3/4","4/4"]`);
            expect(actual, `Output or debug output did not include expected message.\n
stdout: ${output}\nstderr: ${stderrOutput}`).to.be.true;
          });
        });
      });
    });
