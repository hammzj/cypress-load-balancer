...
function createMainLoadBalancingMap(opts: { force?: boolean } = {}) {
    ...
    if (!fs.existsSync(fileName) || opts.force == true) {
        ...
        console.debug('Cypress load balancing file initialized', `Forced: ${opts.force}`)
    }
}
 ...
 */
export function createNewEntry(loadBalancingMap: LoadBalancingMap, testingType: TestingType, filePath: FilePath, opts: {
    force?: boolean
} = {}) {
