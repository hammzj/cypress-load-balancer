...
//  */
// function balanceRunners(specMap: SpecFileCalculationMap, runnerCount: number, filteredFilePaths: Array<FilePath>): Runners {
...
// }
//
// function performLoadBalancing(specMapPath, runnerCount, includeFilePaths, excludeFilePaths, opts = {}): Runners {
...
//     return balanceRunners(specMap, runnerCount, filePaths)
// }


//Lifecycle
//Load specMap and load files based on cypress configuration or launched script arguments
//Add new files, if any exist
//Perform filtering for only files being executed
//Perform load balancing to assign files to runners
//Assign runners for Cypress jobs

//Have the user/GHA run cypress jobs for each runner using the spec file pattern

//For each job:
//Open cypress
//Create empty partial spec map for just those specs
//Test ends: add duration to temp partial spec map of ONLY those specs
//All tests end: save partial spec map file temporarily to merge when all jobs complete

//All jobs end:
//Merge partial spec map files to main spec map file and upload it/save it
